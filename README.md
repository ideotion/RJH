...
Discovery is **end-to-end**: when a feed verifies it is added to your sources and its postings are pulled and scored against your profile, so "tested → verified → scraped" happens for you. It runs two ways. The **Find all feeds & collect** button on the Sources tab tests *every* source in one go, with a live progress bar, and collects from each that succeeds; it runs in the background through the same guards, so it can take a while. The background scheduler also runs a slow, **bounded rolling sweep** (a capped batch of due sources per pass, cached so each is re-probed only every couple of weeks), collecting from new feeds as it goes — auto-collect can be turned off in config (`discovery.auto_collect`). Discovery is honest about what it finds: several major boards (LinkedIn, Indeed, Glassdoor) have **no public feed** and are marked as such — for those, lean on job-alert email instead.
